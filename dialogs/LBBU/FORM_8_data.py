...
def FormOnSetDataEx(uideflist, params):
  ...
  if uideflist.uipData.Dataset.RecordCount==0:
    ...
    s = "select * from %s a, %s b where a.reftype_id=b.reftype_id and b.reference_name='R_JANGKA_LBBU' order by a.refdata_id" % (
    ...
    while not res.Eof:
      rec = ds.AddRecord()
      rec.SetFieldByName('LJANGKA.reference_desc', res.reference_desc)    
      rec.SetFieldByName('LJANGKA.reference_code', res.reference_code)    
      rec.SetFieldByName('LJANGKA.refdata_id', res.refdata_id)    
      res.Next()
    #--
